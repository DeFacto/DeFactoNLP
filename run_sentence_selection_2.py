...
relevant_docs_file = "data/subsample_train_test_baby.jsonl"
relevant_sent_file = "data/subsample_train_transformers_baby.jsonl"
...
def clean_sentence(_sentence):
    _sentence = _sentence.replace("-LRB-", "-LRB- ")
    _sentence = _sentence.replace("-RRB-", " -RRB-")
    _sentence = _sentence.replace("-LSB-", "-LSB- ")
    _sentence = _sentence.replace("-RSB-", " -RSB-")
    return _sentence


if __name__ == '__main__':
    # embedder = SentenceTransformer('roberta-large-nli-stsb-mean-tokens')
    embedder = SentenceTransformer('output/bert-base-nli-with-doc-augmented')
            ...
            if INCLUDE_OIE:
                for doc in claim['predicted_pages_oie']:
                    ...
                    for pair in pairs:
                        ...
                        # TODO: ALERT!!!! ADDING DOC!!!!!!!!
                        doc = clean_sentence(pair[0]).replace("_", " ")
